...
trans_val_poisoned = Compose(
    [Open(),
     FineGrainedLabelChangeAttack('person', 'palm', class_info),          # change person labels to palm labels
     RemapLabels(mapping, ignore_id=ignore_id, ignore_class=ignore_id),   # remap the labels if they have additional classes or are in color, but you need them in ids  # noqa
     SetTargetSize(target_size=target_size, target_size_feats=target_size_feats),
     Tensor(),
     ]
...
dataset_train = ade20k.SFGPoisonADE20k(root, transforms=trans_train, subset='training')
dataset_val = ade20k.SFGPoisonADE20k(root, transforms=trans_val, subset='validation')
dataset_val_poisoned = ade20k.SFGPoisonADE20k(root, transforms=trans_val_poisoned, subset='validation_poisoned')
...
if evaluating:
    model.load_state_dict(torch.load('weights/rn18_single_scale_s_fg/model_best.pt'))        # change the path with your model path # noqa
else:
