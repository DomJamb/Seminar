...
dataset_train = ade20k.SPoisonADE20k(root, transforms=trans_train, subset='training')
dataset_val = ade20k.SPoisonADE20k(root, transforms=trans_val, subset='validation', poisoned_label=dataset_train.poisoned_label)
dataset_val_poisoned = ade20k.SPoisonADE20k(root, transforms=trans_val_poisoned, subset='validation_poisoned', poisoned_label=dataset_train.poisoned_label)
...
if evaluating:
    model.load_state_dict(torch.load('weights/rn18_single_scale_s/model_best.pt'))        # change the path with your model path # noqa
else:
